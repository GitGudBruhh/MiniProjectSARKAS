...
for color in colors:
    ...
    while(i<100):
        ...
        secret_code=gameHelper.generate_random_code(color,5)
        newGameData = GameData(color, 400, 5 , secret_code, 10000, 0)
        print(f"Secret Code: {secret_code}")
        guess_no, comb = genetic_algorithm(secret_code, color, 5, 400, 10000, 0)
        l.append(guess_no)
...
plt.ylabel("Average Guesses (5 slots)")
plt.show()
...
plt.suptitle(eq)
plt.xlabel("Color Count")
plt.ylabel("Average Combinations evaluated (5 slots)")
plt.show()
