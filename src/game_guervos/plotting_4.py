...
for color in colors:
    ...
    while(i<100):
        ...
        secret_code=gameHelper.generate_random_code(color,4)
        newGameData = GameData(color, 400, 4 , secret_code, 10000, 0)
        print(f"Secret Code: {secret_code}")
        guess_no, comb = genetic_algorithm(secret_code, color, 4, 400, 10000, 0)
        l.append(guess_no)
